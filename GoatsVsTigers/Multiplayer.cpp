...
Multiplayer::Multiplayer() {
	...
	gameLoaded = false;
	isComputer = 1;
	goatsPlayerName = "player 1";
	tigersPlayerName = "player 2";
	player1Points = 0;
	player2Points = 0;
}

Multiplayer::Multiplayer(int isComputer, int actualTurn, int goatsRemain, int goatsBeaten, string boardState, string goatsPlayerName, string tigersPlayerName, bool gameLoaded, int player1Points, int player2Points) {
...
string Multiplayer::inputValidation(string message, int pawnType, int operationType) {//operationType: 0 - from, 1 - to
	string mCoord = "";
...
//makes default values again and spaw pawns
void Multiplayer::prepareNextRound() {
	clearScreen();
	...
	while (true) {
		...
		if (returnedValue == -1) {
			...
			if (playerPawns == 0) {//player played goats
				if (player1Points > player2Points) {//player1 won
					winnerStrings[0] = goatsPlayerName +" won!";
				}
				else if (player1Points == player2Points) {//draw
					winnerStrings[0] = "It's a draw!";
				}
				else {//player1
					winnerStrings[0] = tigersPlayerName +" won!";
				}
			}
			else {//player played tigers
				if (player1Points > player2Points) {//player1 lost
					winnerStrings[0] = tigersPlayerName + " won!";
				}
				else if (player1Points == player2Points) {//draw
					winnerStrings[0] = "It's a draw!";
